...
def main(input_filename='Designs.txt'):
    ...
    if os.path.isabs(input_filename) or '/' in input_filename or '\\' in input_filename:
        # It's a full path, use it directly
        input_path = input_filename
    else:
        # It's just a filename, check if it exists in main directory first, then tmp_explore
        main_path = os.path.join(script_dir, input_filename)
        tmp_explore_path = os.path.join(script_dir, 'tmp_explore', input_filename)
        
        if os.path.exists(main_path):
            input_path = main_path
        elif os.path.exists(tmp_explore_path):
            input_path = tmp_explore_path
        else:
            # Default to main directory
            input_path = main_path
    
    # Create directory if it doesn't exist
    os.makedirs(os.path.dirname(input_path), exist_ok=True)
    
    # Read designs from the determined path
    with open(input_path, 'r', encoding='utf-8') as f:
        designs = json.load(f)
    
    print(f"Processing {len(designs)} designs from GeneratedDesigns.txt\n")
    ...
    for i, design in enumerate(designs, 1):
        ...
        if not initial_overhang:
            ...
            if best_label is not None:
                ...
                print("  Updated GeneratedDesigns.txt with refined parameters")
            else:
                print("  No slider could eliminate overhang within bounds.")
        
        print()  # Empty line between designs

    # Write updated designs back to the same file
    with open(input_path, 'w', encoding='utf-8') as f:
        json.dump(designs, f, indent=2)
    print(f"Saved updated designs to {input_path}")
